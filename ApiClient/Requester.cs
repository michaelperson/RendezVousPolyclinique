...
namespace ApiClient
{
    public class PatientRequester : IRequester<PatientModel>
    {
        ...
        public async Task<bool> Patch(string route, JsonPatchDocument<PatientModel> element)
        {
            //1. Seréaliser l'objet en json
            string contentJson = JsonSerializer.Serialize(element);
            //Je préare mon message de request
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Patch, route);
